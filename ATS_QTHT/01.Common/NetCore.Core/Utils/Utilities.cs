...
namespace NetCore.Core.Utils
{
    public partial class Utilities
    {
        ...
        public static string GeneratePostgesql(string query, Dictionary<string, object> parameters)
        {
            var param = "";
            foreach (var item in parameters)
            {
                param = param + $":{item.Key},";
            }
    ...
    public static class Encrypt
    {
        ...
        public static byte[] ConvertFileByteToFileHash(string pType, byte[] bytes)
        {
            string strHash = HashFile(bytes, pType);
            // tạo file txt từ chuỗi hash
            byte[] byteHash = Encoding.ASCII.GetBytes(strHash);
            // file txt chuỗi hash
            return byteHash;
        }
        public static byte[] ConvertFileToFileHash(string pType, IFormFile file)
        {
        ...
        public static string HashFile(IFormFile file, string hashType)
        {
            using (var item = new MemoryStream())
            {
                file.CopyTo(item);
                byte[] bytes = item.ToArray();
                string strHash = HashFile(bytes, hashType);
                return strHash;
            }
        }
        public static string HashFile(byte[] byteFile, string hashType)
        {
            switch (hashType.ToLower().Trim())
            {
                case "sha512":
                    return SHA512_Hash(byteFile);
                case "md5":
                    return Md5_Hash(byteFile);
                case "sha256":
                    return SHA256_hash(byteFile);
                case "sha384":
                    return SHA384_hash(byteFile);
                default:
                    return SHA256_hash(byteFile);
            }
        }

        public static string Md5_Hash(byte[] input)
        {
        ...
        public static string SHA512_Hash(byte[] input)
        {
        ...
        public static string SHA256_hash(byte[] input)
        {
            // Create a SHA256   
        ...
        public static string SHA384_hash(byte[] input)
        {
            // Create a SHA256   
            using (SHA384 sha384Hash = SHA384.Create())
            {
                //From String to byte array
                byte[] hashBytes = sha384Hash.ComputeHash(input);
